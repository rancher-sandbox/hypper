...
func New(logger log.Logger) *EnvSettings {
	...
	env.Trace, _ = strconv.ParseBool(os.Getenv("HYPPER_TRACE"))
...
func (s *EnvSettings) AddFlags(fs *pflag.FlagSet) {
	...
	fs.BoolVarP(&s.Debug, "verbose", "v", s.Trace, "enable verbose output (more verbose than debug)")
...
func (s *EnvSettings) EnvVars() map[string]string {
	return map[string]string{
		...
		"HYPPER_TRACE":    fmt.Sprint(s.Trace),
