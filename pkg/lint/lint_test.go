...
func TestBadChart(t *testing.T) {
	...
	// There should be 5 INFOs, check for them
	var i1, i2, i3, i4, i5 bool
	for _, msg := range m {
		...
		if msg.Severity == support.InfoSev {
			if strings.Contains(msg.Err.Error(), "Setting hypper.cattle.io/shared-dependencies in annotations is optional") {
				i4 = true
			}
		}
		if msg.Severity == support.InfoSev {
			if strings.Contains(msg.Err.Error(), "Setting hypper.cattle.io/optional-dependencies in annotations is optional") {
				i5 = true
			}
		}
	}
