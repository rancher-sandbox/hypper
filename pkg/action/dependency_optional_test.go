...
func newOptionalDepFixture(t *testing.T, ns string) *OptionalDependency {
	od := NewOptionalDependency(actionConfigFixture(t))
	od.Config.SetNamespace(ns)
	return od
}

func TestOptionalDepsList(t *testing.T) {
...
func TestOptionalDepsSetNamespace(t *testing.T) {
	is := assert.New(t)
